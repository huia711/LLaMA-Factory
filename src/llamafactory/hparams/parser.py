...
"""
参数解析与校验（hparams/parser.py）。

这个模块在训练/推理/评测三条链路里都处于“最早期”阶段：
- 训练：`llamafactory-cli train ...` → `train/tuner.py` → `get_train_args(...)`
- 推理（chat/api/webchat）：→ `get_infer_args(...)`
- 评测（eval，若启用）：→ `get_eval_args(...)`

它主要完成：
1) 读取参数来源：CLI / yaml / json（`read_args`）
2) 把参数解析成 dataclass：Model/Data/Training/Finetuning/Generating 等
3) 做强约束校验（尤其是 “某些功能只在推理侧可用”）

与 vLLM 的关键关系（很容易踩坑）：
- 训练 `get_train_args()`：强制 `model_args.infer_backend == EngineName.HF`
  也就是说：**训练阶段不能把 infer_backend 设为 vllm/sglang**。
  vLLM/SGLang 只允许在推理侧（chat/api/web）或离线 batch 推理脚本使用。
- 推理 `get_infer_args()`：允许 infer_backend=vllm，并在此处进行依赖/参数限制检查。
"""
...
def read_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> Union[dict[str, Any], list[str]]:
    ...
    支持三种调用方式：
    - 编程式传参：直接传入 dict 或 list[str]
    - 命令行传 yaml/yml/json：`llamafactory-cli train config.yaml key=value ...`
      - 会用 OmegaConf 将文件配置与 CLI override 合并
    - 纯命令行参数：`llamafactory-cli train --stage sft ...`
    """
    if args is not None:
        return args
...
def _set_env_vars() -> None:
    ...
    例如 NPU 场景下：
    - 关闭/调整 JIT compile 行为
    - 为 vLLM worker 设置更安全的多进程启动方式（spawn）
    """
...
def _check_extra_dependencies(
    ...
    """
    根据启用的特性做“可选依赖”检查（缺依赖时给出明确报错）。

    典型例子：
    - infer_backend=vllm → 要求安装 vllm 且版本满足约束
    - deepspeed → 要求安装 deepspeed 且版本满足约束
    - swanlab/liger-kernel/unsloth 等同理
    """
...
def get_train_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _TRAIN_CLS:
    ...
    # 训练阶段强制使用 HuggingFace backend：
    # - 训练依赖 Trainer/Accelerate/DeepSpeed 等训练栈
    # - vLLM/SGLang 主要是推理引擎，不用于训练主流程
...
def get_infer_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _INFER_CLS:
    """
    推理参数解析入口（chat/api/webchat 等推理侧）。

    与 get_train_args 的关键区别：
    - 允许 `infer_backend=vllm` 或 `infer_backend=sglang`
    - 会对 vLLM 的限制做额外检查（stage/quantization/rope_scaling 等）
    """
    ...
    if model_args.infer_backend == "vllm":
        # vLLM 只支持自回归生成模型（通常对应 SFT 类模型）
        if finetuning_args.stage != "sft":
            raise ValueError("vLLM engine only supports auto-regressive models.")

        # 注意：这里明确禁止 bnb 量化（但允许 GPTQ/AWQ 等 vLLM 支持的量化方式）
        if model_args.quantization_bit is not None:
            raise ValueError("vLLM engine does not support bnb quantization (GPTQ and AWQ are supported).")

        # 注意：vLLM 目前对 RoPE scaling 有限制
        if model_args.rope_scaling is not None:
            raise ValueError("vLLM engine does not support RoPE scaling.")

        # vLLM 只接受单个 adapter（避免多 LoRA 组合造成加载/路由复杂）
        if model_args.adapter_name_or_path is not None and len(model_args.adapter_name_or_path) != 1:
